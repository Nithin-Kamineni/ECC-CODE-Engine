...
# =========================================================
# Hessian-diagonal via Hutchinson estimator (per-weight)
# =========================================================

def compute_hessian_diag(model, loader, device,
                          max_batches: int = 4,
                          n_hutchinson: int = 4,
                          criterion=None
                          ) -> Dict[str, torch.Tensor]:
    """
    Estimate the diagonal of the Hessian of the loss w.r.t. each weight
    using the Hutchinson trick:
        diag(H) = E[v . (H v)]   for Rademacher v in {-1, +1}.
    For each batch:
      - take grad g = dL/dw
      - sample v, compute (H v) by autograd:  d(g . v)/dw
      - accumulate v * (H v) per parameter
    Final per-weight value: mean of v*(Hv) over batches and over hutchinson
    samples.  Take .abs() so it is a non-negative sensitivity score.

    Cost:  ~ (1 + n_hutchinson) backward passes per batch.
    """
    if criterion is None:
        criterion = nn.CrossEntropyLoss()
    model.eval()
    ...
    params = [(name, p) for name, p in _iter_weight_params(model)]
    for i, (x, y) in enumerate(loader):
        if max_batches is not None and i >= max_batches:
            break
        x = x.to(device, non_blocking=True)
        ...
        # First-order grads with create_graph=True so we can backprop through them
        _zero_grads(model)
        loss = criterion(model(x), y)
        grads = torch.autograd.grad(
            loss, [p for _, p in params], create_graph=True, allow_unused=False
        )

        for h in range(n_hutchinson):
            # Sample Rademacher v for each param tensor
            vs = [torch.randint(0, 2, p.shape, device=p.device,
                                dtype=p.dtype) * 2 - 1
                  for _, p in params]
            # Hv via grad of (g . v) w.r.t. params
            gv = sum((g * v).sum() for g, v in zip(grads, vs))
            Hvs = torch.autograd.grad(
                gv, [p for _, p in params],
                retain_graph=(h < n_hutchinson - 1),
                allow_unused=False,
            )
            for (name, _), v, Hv in zip(params, vs, Hvs):
                contrib = (v * Hv).detach()
                if name not in accum:
                    accum[name] = contrib.clone()
                else:
                    accum[name] += contrib
            n_obs += 1

        # Free graph for this batch
        del grads
        torch.cuda.empty_cache()

    out: Dict[str, torch.Tensor] = {}
    for name, t in accum.items():
        out[name] = (t / max(1, n_obs)).abs()
    return out
...
def run_hierarchical(model, loader, device, *,
                     ...
                     hessian_max_batches: int = 4,
                     hessian_n_hutchinson: int = 4,
    ...
    Returns:
      ...
      per_weight     : {'taylor': {layer: tensor}, 'fisher': {...}, 'hessian': {...}}
                       (the per-weight tensors used for selection)
    """
    ...
    if compute_hessian:
        print(f"  [step1c] computing per-weight Hessian-diag (Hutchinson, "
              f"{hessian_n_hutchinson} probes x {hessian_max_batches} batches) ...")
        hessian = compute_hessian_diag(
            model, loader, device,
            max_batches=hessian_max_batches,
            n_hutchinson=hessian_n_hutchinson,
        )
        per_weight["hessian"] = hessian
    ...
    elif layer_metric == "hessian_mean":
        if not hessian:
            raise ValueError("hessian_mean requires --compute-hessian")
        layer_scores = layer_score_from_per_weight(hessian, "mean")
    ...
    for layer_rank, (lname, layer_score) in enumerate(chosen):
        ...
        has_hess = bool(hessian) and (lname in hessian)
        if has_hess:
            hes_flat = hessian[lname].detach().cpu().flatten()
        ...
        for in_layer_rank, loc in enumerate(top_idx.tolist()):
            ...
            if has_hess:
                row["hessian"] = float(hes_flat[int(loc)])
...
def write_rows_csv(rows: List[dict], out_csv: str, extra_cols: List[str]):
    ...
    if rows and "hessian" in rows[0]:
        base_cols.append("hessian")
...
def fresh_model(arch, dataset, num_classes, use_pretrained,
                weights_path, device, fmt: str = "float32", qtag: str = "ptq",
                qtag_suffix: str = ""):
    m = TQ.build_model(arch, num_classes, use_pretrained=bool(use_pretrained), dataset=dataset).to(device)
    if weights_path:
    ...
    elif qtag == "qat" and fmt.startswith("int"):
        nb = int(fmt[3:])
        ck = TQ.ckpt_path(dataset, arch, f"int{nb}_qat{qtag_suffix}")
        if os.path.exists(ck):
            ...
            if "state_dict" in payload and payload.get("meta", {}).get("qat_baked", False):
                ...
                TQ.load_quantized_into_model(
                    m, dataset, arch, nb, qtag="qat", map_location=device)
                print(f"  [weights] source=artifact_int{nb}_qat (legacy PTQ-payload format)")
        else:
            print(f"  [warn] no int{nb}_qat checkpoint found, "
                  f"falling back to float32 + on-the-fly PTQ")
            src, sz = TQ.maybe_load_float32_or_pretrained(
                m, dataset, arch,
                use_pretrained=bool(use_pretrained), map_location=device)
...
def run_one_arch(arch: str, args):
    ...
    for fmt in formats:
        ...
        m = fresh_model(arch, args.dataset, nc,
                        ...
                        fmt=fmt, qtag=args.qtag, qtag_suffix=args.qtag_suffix)
        extra = None
        if fmt.startswith("int") and args.qtag != "qat":
            nbits = int(fmt[3:])
        ...
        rows, layer_scores, _ = run_hierarchical(
            ...
            hessian_max_batches=args.hessian_max_batches,
            hessian_n_hutchinson=args.hessian_n_hutchinson,
        )
...
def main():
    ...
    p.add_argument("--qtag", default="ptq", choices=["ptq", "qat"],
                   help="Quantization checkpoint variant. 'ptq' (default) "
                        "quantizes float32 weights on-the-fly. 'qat' loads "
                        "the int{N}_qat checkpoint from qat_train.py.")
    p.add_argument("--qtag-suffix", default="",
                   help="Optional suffix on QAT checkpoint name, e.g. _lsq loads "
                        "model_int4_qat_lsq.pth. Must match qat_train.py's flag.")

    p.add_argument("--layer-metric",
                   default="grad_norm",
                   choices=["grad_norm", "taylor_mean", "fisher_mean",
                            "hessian_mean", "perturb"],
    ...
    # ---- Hessian-diagonal options (Hutchinson estimator) ----
    p.add_argument("--compute-hessian", type=int, default=0,
                   help="If 1, also compute per-weight Hessian-diagonal "
                        "via Hutchinson and add a 'hessian' column to the CSV. "
                        "Required if --layer-metric=hessian_mean.")
    p.add_argument("--hessian-max-batches", type=int, default=4,
                   help="Number of batches to use for Hessian-diag estimation.")
    p.add_argument("--hessian-n-hutchinson", type=int, default=4,
                   help="Number of Hutchinson probe vectors per batch.")

    p.add_argument("--out-dir", default="artifacts/sensitivity")
    args = p.parse_args()
    args.dataset = args.dataset.upper()

    # auto-enable hessian computation if it's needed for the layer ranking
    if args.layer_metric == "hessian_mean" and not args.compute_hessian:
        args.compute_hessian = 1
        print("[info] auto-enabled --compute-hessian (required for layer-metric=hessian_mean)")
