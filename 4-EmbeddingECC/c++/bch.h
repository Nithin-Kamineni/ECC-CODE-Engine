...
// ============================================================
// Load a PMatrix from a Python-exported .npy file (int8, shape k×r).
//
// Usage: pass the file generated by export_parity_matrix.py so that
// C++ and Python use the same galois.BCH parity matrix, ensuring
// bit-exact compatibility between both pipelines.
// ============================================================
inline PMatrix pmatrix_from_npy(const std::string& path) {
    NpyInfo info = npy_read_header(path);
    if (info.shape.size() != 2)
        throw std::runtime_error(
            "Parity matrix .npy must be 2D (shape k×r), got "
            + std::to_string(info.shape.size()) + "D: " + path);

    size_t k = info.shape[0];
    size_t r = info.shape[1];

    // Load flat int8 data (values should be 0 or 1)
    std::vector<int8_t> flat = npy_load_int8(path);
    if (flat.size() != k * r)
        throw std::runtime_error(
            "Parity matrix element count mismatch: expected "
            + std::to_string(k * r) + " got " + std::to_string(flat.size())
            + " in " + path);

    std::vector<std::vector<uint8_t>> P_raw(k, std::vector<uint8_t>(r));
    for (size_t i = 0; i < k; i++)
        for (size_t j = 0; j < r; j++)
            P_raw[i][j] = static_cast<uint8_t>(flat[i * r + j] & 1);

    return PMatrix(P_raw);
}
